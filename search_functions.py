...
def format_red_book_block(index, result):
    lines = [f"{index}:"]
    if result.get("headword"):
        lines.append(format_labeled_line("Headword:", result["headword"]))
    lines.append(format_labeled_line("Indonesian:", result["indonesian"]))
    lines.append(format_labeled_line("English:", result["english"]))
    if result.get("page"):
...
def search_matching_red_book_examples(query, limit, include_general=True):
    if not should_index_red_book():
        return []
    if not red_book_index.is_red_book_index_valid(red_book_pdf_path(), red_book_cache_path()):
        return []
    return red_book_index.search_red_book_examples(
        query,
        limit,
        red_book_pdf_path(),
        red_book_cache_path(),
